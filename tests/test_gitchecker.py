...
class TestGitChecker(unittest.TestCase):
    ...
    def test_check_and_update(self):
        ...
        for data in ext_data:
            ...
            if data.filename == "jansson.git":
                ...
                self.assertIsNotNone(data.new_version.tag)
                self.assertEqual(data.current_version.branch, data.new_version.tag)
                self.assertEqual(data.current_version.commit, data.new_version.commit)
    ...
    def _test_update_data(self, data, orig_source):
        ...
        if data.filename == "protobuf-c.git":
            ...
            self.assertNotEqual(data.source, orig_source)
            self.assertEqual(data.source["commit"], orig_source["commit"])
            self.assertIn("branch", orig_source)
            self.assertNotIn("tag", orig_source)
            self.assertIn("tag", data.source)
            self.assertNotIn("branch", data.source)
            self.assertEqual(orig_source["branch"], data.source["tag"])
